...
WUPS_GET_CONFIG() {
    ...
    WUPSConfig_CreateHandled(&config, "Innoverse-Patcher");

    WUPSConfigCategoryHandle patching_cat;
    WUPSConfig_AddCategoryByNameHandled(config, "Patching", &patching_cat);

    WUPSConfigItemBoolean_AddToCategoryHandled(config, patching_cat, "connect_to_network", "Connect to Innoverse", Config::connect_to_network, &connect_to_network_changed);
